...
2. **Evidence before done**: never claim something works without running it
   ...
   **Assert the property the artifact must have — not a string describing it — and assert
   it across every output target, not the one you happened to render.** "Open it and look"
   is the tie-breaker for properties you cannot yet name, not the rule: looking costs
   O(artifacts you thought to open), and you will open the one that works. Scored against
   four real defects on 2026-07-16, looking caught 1; `assert tick ∈ data` caught 4.
   The failures this is distilled from, all in one session: `timeAxis: true` was true while
   the axis was wrong; a tick test asserted the label's *shape*, which passes whether every
   tick sits on data or none does; every fixture was built from the *change's intent*, so it
   could only confirm the change — the date list tested the shapes the whitelist listed, the
   encoding floor was tested at one below itself, the tick fix only on the 1st of the month
   (the one day it can't fail), the font clamp only at export sizes and never at preview
   sizes, where it broke. **A "must keep working" list is generated by running the code you
   are about to change, and enumerating what works today.**
